...
func (d *signupsDao) updateDataByID(ctx context.Context, db *gorm.DB, table *model.Signups) error {
	...
	if table.SignupInfo != nil {
		update["signup_info"] = table.SignupInfo
	}
	if table.Priority != 0 {
		update["priority"] = table.Priority
	}
	if table.IsRich != false /*Warning: if the value itself is false, can't be updated*/ {
		update["is_rich"] = table.IsRich
	}
	if table.IsProxy != false /*Warning: if the value itself is false, can't be updated*/ {
	...
	if table.IsDove != false /*Warning: if the value itself is false, can't be updated*/ {
		update["is_dove"] = table.IsDove
	}
	if table.IsCandidate != false /*Warning: if the value itself is false, can't be updated*/ {
