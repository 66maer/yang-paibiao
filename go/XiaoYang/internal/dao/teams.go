...
func (d *teamsDao) updateDataByID(ctx context.Context, db *gorm.DB, table *model.Teams) error {
	...
	if table.TeamTime.IsZero() == false {
		update["team_time"] = table.TeamTime
	}
	if table.Dungeons != "" {
		update["dungeons"] = table.Dungeons
	}
	if table.BookXuanjing != false /*Warning: if the value itself is false, can't be updated*/ {
		update["book_xuanjing"] = table.BookXuanjing
	}
	if table.BookYuntie != false /*Warning: if the value itself is false, can't be updated*/ {
		update["book_yuntie"] = table.BookYuntie
	}
	if table.IsVisible != false /*Warning: if the value itself is false, can't be updated*/ {
		update["is_visible"] = table.IsVisible
	}
	if table.IsLock != false /*Warning: if the value itself is false, can't be updated*/ {
	...
	if table.CloseTime.IsZero() == false {
