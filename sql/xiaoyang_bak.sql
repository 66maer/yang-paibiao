...
-- 添加群组表注释
COMMENT ON TABLE guild IS '群组表';
COMMENT ON COLUMN guild.id IS '群组ID';
COMMENT ON COLUMN guild.group_number IS '群号';
COMMENT ON COLUMN guild.ukey IS '群组唯一标识';
COMMENT ON COLUMN guild.name IS '群组名';
COMMENT ON COLUMN guild.server IS '群组所在服务器';
COMMENT ON COLUMN guild.avatar IS '群组头像';
COMMENT ON COLUMN guild.expire_time IS '过期时间';
COMMENT ON COLUMN guild.preferences IS '群组偏好设置';

-- 创建群组表索引
CREATE INDEX idx_guild_group_number ON guild(group_number);
CREATE INDEX idx_guild_ukey ON guild(ukey);
...
CREATE TABLE guild_members (
    ...
    FOREIGN KEY (guild_id) REFERENCES guild(id) ON DELETE CASCADE,  -- 外键关联群组表, 级联删除
...
COMMENT ON COLUMN logs.target_id IS '操作对象ID';
