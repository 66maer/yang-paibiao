...
const EditHistoryRecordModal = ({ visible, onClose, onSuccess, teamData }) => {
  ...
  const initializeForm = () => {
    try {
      const parsedSummary = teamData.summary ? JSON.parse(teamData.summary) : {};
      const teamDate = teamData.createTime || teamData.teamTime;

      const formValues = {
        date: teamDate ? dayjs(teamDate) : dayjs(),
        dungeons: teamData.dungeons,
        notice: teamData.notice,
        salary: parsedSummary.salary || 0,
        perPersonSalary: parsedSummary.perPersonSalary || 0,
        special_drops: parsedSummary.specialDrops || [],
        workersCount: parsedSummary.workersCount || 20,
        blacklist: parsedSummary.blacklist || "",
      };

      form.setFieldsValue(formValues);
      setWorkersCount(formValues.workersCount);
      setSelectedTags(formValues.special_drops);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const updateTeamPayload = {
        ...
        rule: teamData.rule || JSON.stringify(Array(25).fill({})),
        notice: values.notice || "",
        guildId: store.getState().guild.guildId,
        createrId: teamData.createrId,
        isLock: teamData.isLock || true,
        isHidden: teamData.isHidden || false,
        bookXuanjing: teamData.bookXuanjing || false,
        bookYuntie: teamData.bookYuntie || false,
      };

      const updateRes = await request.post("/team/updateTeam", updateTeamPayload);

      if (updateRes.code !== 0) {
        throw new Error(updateRes.msg || "更新团队信息失败");
      }

      // 第二步：更新团队记录（使用CloseTeam接口，添加isEdit标识）
      ...
      if (closeRes.code !== 0) {
        throw new Error(closeRes.msg || "更新团队记录失败");
      }
...
export default EditHistoryRecordModal;
