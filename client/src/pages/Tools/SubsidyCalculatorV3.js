...
const SubsidyCalculatorV3 = () => {
  ...
  const DEFAULT_ITEMS = [
    ...
    { id: "water-fish", name: "水煮鱼", people: 1, expectedAmount: 1000, isFixed: false, isDeletable: true },
    { id: "jade-flute", name: "玉笛", people: 1, expectedAmount: 1500, isFixed: false, isDeletable: true },
    { id: "twenty-four", name: "二十四", people: 1, expectedAmount: 1500, isFixed: false, isDeletable: true },
    { id: "tongze", name: "同泽", people: 1, expectedAmount: 500, isFixed: false, isDeletable: true },
  ];
    ...
    if (calculateMode === "proportion") {
        ...
        return subsidyItems.map(item => {
          ...
          if (item.isFixed) {
            ...
            return {
              ...
              actualUnitAmount,
              totalActualAmount,
              deductionInfo: deduction !== 0 ? `${isNegative ? '负补贴' : '扣除'}${Math.abs(deduction)}，${deductionPercent}%` : null,
              hasError: hasFixedError
            };
          }
        });
      ...
      return subsidyItems.map(item => {
        ...
        if (item.isFixed) {
          ...
          const deductionPercent = originalUnitAmount > 0 ? ((actualDeduction / originalUnitAmount) * 100).toFixed(1) : 0;
          const isNegative = actualUnitAmount < 0;
          return {
            ...item,
            actualUnitAmount,
            totalActualAmount,
            deductionInfo: actualDeduction !== 0 ? `${isNegative ? '负补贴' : '扣除'}${Math.abs(actualDeduction)}，${deductionPercent}%` : null,
            hasError: hasFixedError
          };
        }
      });
  ...
  const updateSubsidyItem = (id, field, value) => {
    setSubsidyItems(prev => prev.map(item =>
      ...
      render: (value, record) => (
        <div>
          ...
          {record.deductionInfo && (
            <div style={{ fontSize: "11px", color: "#ff7875", lineHeight: 1.2 }}>
              （{record.deductionInfo}）
            </div>
          )}
        </div>
