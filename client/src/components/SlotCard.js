  ...
  const contentXinfa = (allow_xinfa_list) => {
    ...
    const commonXinfaGroup = (name, ref_list, url, content) => {
      if (
        ...
        allow_xinfa_list.every((xinfa) => xinfaInfoTable[xinfa].type.includes(name))
      ) {
        return (
    ...
    if (allow_xinfa_list.length >= allXinfaList.length - 6 && allow_xinfa_list.length < allXinfaList.length) {
      const xinfa_complement = allXinfaList.filter((xinfa) => !allow_xinfa_list.includes(xinfa));
      ...
      <div className="slot-signup-content">
        <div className="header">
          ...
          <div className="labels">
            {client_type === "无界" && label("/mobile.svg", "无界", "使用无界端出战")}
        ...
        <div className="footer">{is_proxy && <span>{submit_name}(代报)</span>}</div>
      </div>
    </div>
  );
};
...
const SlotCardCanvas = ({ rules = {}, signup_info }) => {
  if (!signup_info || Object.keys(signup_info).length === 0) {
    return <div className="slot-card-canvas slot-rule">{rulesContent(rules)}</div>;
  }
  return (
