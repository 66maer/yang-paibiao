...
import { getGuildDungeonOptions, updateGuildDungeonOptions } from "@/api/guildConfigs";
import { showSuccess, showError, showConfirm } from "@/utils/toast.jsx";

export default function GuildDungeonConfigPage() {
  ...
  // 获取副本配置
  const { data, error, mutate } = useSWR("guild-dungeon-options", () => getGuildDungeonOptions(), {
  ...
  const handleDelete = async (index) => {
    ...
    try {
      await updateGuildDungeonOptions(reorderedDungeons);
  ...
  const handleSave = async () => {
    ...
    if (editMode === "add") {
      ...
      newDungeons = dungeons.map((d, i) => (i === editingIndex ? { ...editForm } : d));
    }

    try {
      await updateGuildDungeonOptions(newDungeons);
  ...
  return (
    <div>
      <Card>
        <CardHeader className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">管理本群的副本选项，支持拖拽排序</p>
          </div>
        ...
        <CardBody>
          {displayDungeons.length === 0 ? (
            <div className="text-center text-gray-500 py-8">暂无副本配置，点击右上角添加</div>
          ) : (
            <Reorder.Group axis="y" values={displayDungeons} onReorder={handleReorder} className="space-y-2">
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          <ModalHeader>{editMode === "add" ? "新增副本" : "编辑副本"}</ModalHeader>
          <ModalBody>
            <Input
              label="副本名称"
              value={editForm.name}
