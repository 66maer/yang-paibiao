...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        CREATE INDEX idx_users_other_nicknames ON users USING GIN (other_nicknames);
    """)
    ...
    op.execute("""
        CREATE TABLE character_players (
            ...
            relation_type VARCHAR(20) NOT NULL DEFAULT 'owner',
    ...
    op.execute("""
        CREATE TABLE signups (
            ...
            signup_user_id INT REFERENCES users(id),
            signup_character_id INT REFERENCES characters(id),
            signup_info JSONB,
    ...
    op.execute("""
        CREATE TABLE gold_team_records (
            ...
            total_gold DECIMAL(12, 2) DEFAULT 0,
            worker_count INT DEFAULT 0,
            special_drops TEXT[],
            heibenren_user_id INT REFERENCES users(id),
            heibenren_character_id INT REFERENCES characters(id),
            heibenren_info JSONB,
            notes TEXT,
            creator_id INT NOT NULL REFERENCES users(id),
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
def downgrade() -> None:
    # 删除所有触发器
    op.execute("DROP TRIGGER IF EXISTS update_gold_team_records_updated_at ON gold_team_records;")
    op.execute("DROP TRIGGER IF EXISTS update_signups_updated_at ON signups;")
    op.execute("DROP TRIGGER IF EXISTS update_team_templates_updated_at ON team_templates;")
    op.execute("DROP TRIGGER IF EXISTS update_teams_updated_at ON teams;")
    op.execute("DROP TRIGGER IF EXISTS update_guild_members_updated_at ON guild_members;")
    op.execute("DROP TRIGGER IF EXISTS update_guilds_updated_at ON guilds;")
    op.execute("DROP TRIGGER IF EXISTS update_character_players_updated_at ON character_players;")
    op.execute("DROP TRIGGER IF EXISTS update_characters_updated_at ON characters;")
    op.execute("DROP TRIGGER IF EXISTS update_users_updated_at ON users;")
    op.execute("DROP TRIGGER IF EXISTS update_guild_subscriptions_updated_at ON guild_subscriptions;")
    op.execute("DROP TRIGGER IF EXISTS update_system_admins_updated_at ON system_admins;")

    # 删除触发器函数
    op.execute("DROP FUNCTION IF EXISTS update_updated_at_column();")

    # 按依赖顺序删除表
    op.execute("DROP TABLE IF EXISTS gold_team_records CASCADE;")
    op.execute("DROP TABLE IF EXISTS signups CASCADE;")
    op.execute("DROP TABLE IF EXISTS team_templates CASCADE;")
    op.execute("DROP TABLE IF EXISTS teams CASCADE;")
    op.execute("DROP TABLE IF EXISTS guild_members CASCADE;")
    op.execute("DROP TABLE IF EXISTS guild_subscriptions CASCADE;")
    op.execute("DROP TABLE IF EXISTS character_players CASCADE;")
    op.execute("DROP TABLE IF EXISTS guilds CASCADE;")
    op.execute("DROP TABLE IF EXISTS characters CASCADE;")
    op.execute("DROP TABLE IF EXISTS users CASCADE;")
    op.execute("DROP TABLE IF EXISTS system_admins CASCADE;")
