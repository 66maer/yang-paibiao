...
class BotTeamDetail(BaseModel):
    ...
    slot_view: Optional[List[int]] = Field(None, description="坑位视图（已废弃）")
    slot_assignments: Optional[List[dict]] = Field(None, description="坑位分配情况")
    waitlist: Optional[List[int]] = Field(None, description="候补列表")
...
class BotCharacterSimple(BaseModel):
    ...
    xinfa: str = Field(..., description="主心法")
