...
| Secret 名称       | 说明             | 示例值                                   | 必需                 |
| ----------------- | ---------------- | ---------------------------------------- | -------------------- |
| SSH_HOST          | 服务器 IP 或域名 | `192.168.1.100`                          | ✅ 必需              |
| SSH_USER          | SSH 用户名       | `maer`                                   | ✅ 必需              |
| SSH_PRIVATE_KEY   | SSH 私钥内容     | `-----BEGIN OPENSSH PRIVATE KEY-----...` | ✅ 必需              |
| SERVER_PORT       | SSH 端口         | `22`（默认）                             | ⚪ 可选              |
| VITE_API_BASE_URL | 前端 API 地址    | `/api/v2`                                | ⚪ 可选              |
| GHCR_TOKEN        | GHCR 拉取令牌    | `ghp_xxx`                                | 私有 GHCR 镜像时必需 |
| GHCR_USERNAME     | GHCR 用户名      | `66maer`                                 | ⚪ 可选              |

**注意：** `GITHUB_TOKEN` 由 GitHub Actions 自动提供，不需要手动添加。

**注意：** 服务器拉取私有 GHCR 镜像时，不能使用 GitHub Actions 自带的 `GITHUB_TOKEN` 直接代替服务器登录。请额外配置 `GHCR_TOKEN`，并确保它至少有 `read:packages` 权限。
...
# 在本地机器上
cat ~/.ssh/id_rsa
# 复制整个输出（包括 BEGIN 和 END 行）到 SSH_PRIVATE_KEY
```
